...
export function StorefrontFooter() {
  ...
  const handleSubscribe = async () => {
    ...
    try {
      const res = await subscribeNewsletter({ email: trimmed }).unwrap();
      toast.success(res.message || 'Subscribed successfully!');
      setEmail('');
    } catch (err: any) {
      const msg = err?.data?.message || 'Subscription failed. Please try again.';
      toast.error(msg);
    }
  };
        ...
        <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-20 py-10 flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="flex flex-col gap-3 w-full max-w-sm">
            ...
            <button
              ...
              disabled={isLoading}
              className="w-full bg-white text-black rounded-full py-3 text-sm font-semibold hover:bg-gray-100 transition-colors disabled:opacity-60 flex items-center justify-center gap-2"
            >
              {isLoading ? (
      ...
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-20">
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-8 border-b border-gray-300 pb-10">
          ...
          <div className="col-span-2 md:col-span-1 flex flex-col gap-4">
            <h2 className="text-2xl font-extrabold" style={{ fontFamily: "'Integral CF', 'Inter', sans-serif" }}>
              SHOP.CO
            </h2>
            ...
            <div className="flex items-center gap-3 mt-1">
              {[
                { img: '/images/21.png', alt: 'Twitter' },
                { img: '/images/22.png', alt: 'Facebook' },
                { img: '/images/23.png', alt: 'Instagram' },
                { img: '/images/89.png', alt: 'GitHub' },
              ].map(({ img, alt }) => (
                <Link key={alt} href="#" aria-label={alt}
          ...
          {(['company', 'help', 'faq', 'resources'] as const).map((key) => (
            <div key={key} className="flex flex-col gap-3">
              <h4 className="font-bold text-sm tracking-widest uppercase">{key}</h4>
              {footerLinks[key].map((item) => (
                <Link key={item} href="#" className="text-sm text-gray-600 hover:text-black transition-colors">{item}</Link>
              ))}
            </div>
          ))}
        </div>
